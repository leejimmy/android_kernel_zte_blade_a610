		...
		if (ret == SUCCESS) {
			GTP_DEBUG
			    ("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
			     ...
			     opr_buf[0], opr_buf[0]);
			//modify@zte.com.cn modify at 20160519 begin
			if ((opr_buf[0] < 90) && (opr_buf[0] >= send_cfg_buf[sensor_id][0]) ) {
			...
			//modify@zte.com.cn modify at 20160519 end
		} else {
...
#endif
//modify@zte.com.cn add at 20160427 begin
#ifdef CONFIG_WIND_DEVICE_INFO  
	extern u16 g_ctp_fwvr;
   	extern u16 g_ctp_vendor;
   	extern char g_ctp_id_str[21];
	u8 temp_data[10]; //store device info 
#endif
//modify@zte.com.cn add at 20160427 end
	...
	//modify@zte.com.cn modify at 20160518 begin
	if (ret < 0){
		GTP_ERROR("GTP init panel failed.");
		return -1;	
	}
	//modify@zte.com.cn modify at 20160518 end
...
//modify@zte.com.cn add at 20160427 begin 
   #ifdef CONFIG_WIND_DEVICE_INFO  
	i2c_read_bytes(client, GTP_REG_VERSION, temp_data, 6);
	if(temp_data[3] == 0){
		sprintf(g_ctp_id_str, "GT%c%c%c_%02X%02X", temp_data[0], temp_data[1], 
			temp_data[2], temp_data[5], temp_data[4]);
	}else{
		sprintf(g_ctp_id_str, "GT%c%c%c%c_%02X%02X", temp_data[0], temp_data[1], 
			temp_data[2], temp_data[3], temp_data[5], temp_data[4]);	
	}	

	i2c_read_bytes(client, GTP_REG_SENSOR_ID, temp_data, 1);
	g_ctp_vendor = temp_data[0];

	i2c_read_bytes(client, GTP_REG_CONFIG_DATA, temp_data, 1);
...
static void tpd_resume(struct device *h)
{
	...
	doze_status = DOZE_DISABLED;
#else
	printk("resume 000000000\n");

	/* mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); */
	enable_irq(gt_touch_irq);
	printk("resume 111111\n");
#endif
